...
# Octopus Series Firmware (STM32H723)

Octopus with the `STM32H723` MCU 
...
<Tabs groupId="octopus-make-connect">
    <TabItem value="bridge" label="CAN Bridge Firmware" default>
        Build the `Octopus` CAN bridge firmware using the `STM32H723` as the `MCU` according to the following options
        <ImageView
    ...
    <TabItem value="usb" label="USB Serial Firmware">
        Build the `Octopus` USB serial firmware using the `STM32H723` as the `MCU` according to the following options
        <ImageView
